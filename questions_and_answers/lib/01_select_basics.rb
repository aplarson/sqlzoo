...
def per_capita_gdp
  ...
  execute(<<-SQL)
    SELECT name, (gdp/population) AS per_capita FROM countries WHERE area > 5000000;
  SQL
end
...
def just_the_right_size
  ...
  execute(<<-SQL)
    SELECT name, (area / 1000) AS k_sq_km FROM countries WHERE area BETWEEN 200000 AND 250000;
  SQL
end
